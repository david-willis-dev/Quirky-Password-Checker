...
// Calculates the jaroDistance between two strings:
...
// Calculates the quirk distance, which goes into the quirk score calculations
...
// Set the score of the password based on uppercase, special characters, numbers, and lowercase:
float scoreSet(string password) {
...
// Adds points to the score based off of the position in the original file! (lower position in file will add to score)
double scorePosition(int position) {
    int score = 0;
    if(position > 25000)
        score += 1;
